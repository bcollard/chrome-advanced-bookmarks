...
![Advanced Bookmarks screenshot](screenshots/image%20copy%205.png)
...
- **Localized** — UI and store listing in English, French, German, Spanish, Portuguese (BR), Japanese and Chinese (Simplified)
...
├── popup.html             # Dialog UI (English defaults + data-i18n hooks)
├── popup.css              # Styles (Google-inspired design)
├── popup.js               # All logic — vanilla JS, no bundler
├── _locales/              # UI + store name/summary, one dir per language
│   ├── en/messages.json   # default_locale — the reference key set
│   ├── fr/ de/ es/ pt_BR/ ja/ zh_CN/
├── icons/
│   ├── icon16.png         # Generated by scripts/generate-icons.py
│   ├── icon48.png
│   └── icon128.png
│
│  ── listing material, excluded from the .zip ────────────
├── docs/                  # GitHub Pages landing page (+ privacy policy)
├── promo/                 # Store promo tiles: HTML sources + rendered PNGs
...
1. Copy `_locales/en/messages.json` to `_locales/<locale>/messages.json` and translate every `message` value — keep the keys identical
2. Copy `store-listing/en.md` to `store-listing/<locale>.md` and translate it
3. Run `make validate` — it fails if any locale is missing a key, or if a name or summary busts the store's limits
4. Paste the detailed description into the dashboard under that language (see [PUBLISHING.md](PUBLISHING.md))
